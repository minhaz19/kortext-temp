...
const DeliveryOption = props => {
    ...
    const continueProcess = e => {
        ...
        if(!formValues.deliveryOption?.city || !formValues.deliveryOption?.fee || !formValues.deliveryOption?.instruction){
            !formValues.deliveryOption?.city && setErrors({...errors, city: 'City is required!'})
            !formValues.deliveryOption?.fee && setErrors({...errors, fee: 'Price is required!'})
            !formValues.deliveryOption?.instruction && setErrors({...errors, instruction: 'Instruction is required!'})
    ...
    const deliveryCityNameChange = e => {
        ...
        dispatch(changeFormValueAction({name: 'deliveryOption', value: {...formValues.deliveryOption, city: value}}))
    }
    ...
    const deliveryFeeChange = e => {
        ...
        dispatch(changeFormValueAction({name: 'deliveryOption', value: {...formValues.deliveryOption, fee: value}}))
    }
    ...
    const deliveryInstructionChange = e => {
        ...
        dispatch(changeFormValueAction({name: 'deliveryOption', value: {...formValues.deliveryOption, instruction: value}}))
    }
        ...
        <div className="md:flex justify-center items-center min-h-screen bg-gray-100 px-8 md:px-0">
            <div className="flex-col grid md:grid-cols-2 gap-10">
                <div className="pl-4 pt-10 mt-32 md:mt-0">
                    <p className="text-yellow-400 text-xs">6.1 of 10</p>
                            ...
                            <div className="mb-3 md:px-5 pt-0 relative">
                                <div className="w-4 max-h-10 absolute md:-left-0 -left-7 top-2 ">
                                    <div className="w-5 h-5 bg-yellow-400 rounded-full flex justify-center items-center">
                                        <p className="text-white text-xs">6.1</p>
                                    </div>
                                    <img className="h-28 w-4 rounded" src='/assets/svgs/createshop/line.svg' alt=""/>
                                </div>
                                <h3 className="py-2 text-lg font-medium text-center">Add a delivery option</h3>
                                <h3 className="py-2">City Name</h3>
                                <input
                                    type="text"
                                    value={formValues.deliveryOption?.city || ''}
                                ...
                                <input
                                    type="number"
                                    value={formValues?.deliveryOption?.fee || ''}
                                    name="price"
                                ...
                                <input
                                    type="text"
                                    name="instruction"
                                    value={formValues.deliveryOption?.instruction || ''}
